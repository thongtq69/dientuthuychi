...
export function SectionHeading({ eyebrow, title, description, actionLabel = 'Xem thêm', actionHref = '#', compact = false }) {
  return (
    <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
      <div className="max-w-3xl">
        {eyebrow ? <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-[#1b66d2]">{eyebrow}</p> : null}
        <h2 className={`mt-1 font-bold tracking-normal text-slate-950 ${compact ? 'text-base sm:text-lg' : 'text-lg sm:text-xl'}`}>{title}</h2>
